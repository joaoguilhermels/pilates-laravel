    ...
    <div class="mx-auto max-w-7xl px-4 pt-20 pb-16 sm:px-6 lg:px-8 lg:pt-32">
      <div class="text-center">
        <h1 class="mx-auto max-w-4xl text-5xl font-bold tracking-tight text-slate-900 sm:text-7xl">
          {{ __('app.pilates_studio_made_simple') }} 
          <span class="relative whitespace-nowrap text-blue-600">
            ...
            <span class="relative">{{ __('app.made_simple') }}</span>
          </span>
          {{ __('app.for_modern_studios') }}
        </h1>
        <p class="mx-auto mt-6 max-w-2xl text-lg tracking-tight text-slate-700">
          {{ __('app.homepage_subtitle') }}
        </p>
        <div class="mt-10 flex justify-center gap-x-6">
          @guest
            <a href="{{ route('register') }}" class="inline-flex items-center rounded-md bg-blue-600 px-6 py-3 text-white hover:bg-blue-500 font-semibold">{{ __('app.start_free_trial') }}</a>
          ...
          @else
            <a href="{{ route('home') }}" class="inline-flex items-center rounded-md bg-blue-600 px-6 py-3 text-white hover:bg-blue-500 font-semibold">{{ __('app.go_to_dashboard') }}</a>
          @endguest
        </div>
        <div class="mt-36 lg:mt-44">
          <p class="text-base text-slate-900 font-semibold">{{ __('app.trusted_by_studios_worldwide') }}</p>
          <div class="mt-8 flex items-center justify-center gap-x-12 opacity-60">
            ...
            <div class="text-2xl font-bold text-slate-400">Movimento Vital</div>
          </div>
        </div>
      </div>
    </div>
