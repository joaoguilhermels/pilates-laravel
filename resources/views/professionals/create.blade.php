...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
  ...
  <x-smart-breadcrumbs :items="[
    ['title' => __('app.professionals'), 'url' => route('professionals.index')],
  ...
  <div class="mb-8">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ __('app.create_new_professional') }}</h1>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('app.add_instructor_staff_member') }}</p>
      </div>
  ...
  <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
    <form action="{{ route('professionals.store') }}" method="POST" class="space-y-6 p-6">
      @csrf
      @include('professionals.form', ['submitButtonText' => __('app.create_professional')])
    </form>
  </div>
</div>
@endsection
