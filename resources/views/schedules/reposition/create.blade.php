...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
  ...
  <div class="mb-8">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">{{ __('app.create_new_reposition') }}</h1>
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ __('app.reposition_class_session') }}</p>
      </div>
  ...
  <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
    <form action="{{ route('schedules.reposition.store') }}" method="POST" class="space-y-6 p-6">
      ...
      <!-- Client Selection -->
      <div>
        <label for="client_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('app.client') }} *</label>
        <div class="mt-1">
      ...
      <div>
        <label for="class_type_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('app.class_type') }} *</label>
        <div class="mt-1">
          <select name="class_type_id" id="class_type_id" 
                  ...
                  required>
            <option value="">{{ __('app.select_class_type') }}</option>
            @if(isset($classTypes))
      ...
      <div>
        <label for="professional_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('app.professional') }} *</label>
        <div class="mt-1">
          <select name="professional_id" id="professional_id" 
                  ...
                  required>
            <option value="">{{ __('app.select_professional') }}</option>
      ...
      <!-- Room Selection -->
      <div>
        <label for="room_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('app.room') }} *</label>
        <div class="mt-1">
      ...
      <div>
        ...
        <div class="mt-1">
          <textarea name="observation" id="observation" rows="3" 
                    ...
                    placeholder="{{ __('app.notes_reposition_placeholder') }}">{{ old('observation') }}</textarea>
        </div>
        @error('observation')
          <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
        @enderror
