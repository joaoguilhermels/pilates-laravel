...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
  ...
  <div class="mb-8">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Update {{ $plan->name }}'s information.</p>
      </div>
  ...
  <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700">
    <form action="{{ route('plans.update', $plan) }}" method="POST" class="space-y-6 p-6">
      @csrf
      @method('PATCH')
      @include('plans.form', ['submitButtonText' => 'Update Plan', 'plan' => $plan, 'classTypes' => $classTypes ?? []])
    </form>
  </div>
</div>
@endsection
